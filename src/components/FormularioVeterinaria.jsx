...
const FormularioVeterinaria = () => {
    const citasLocalStorage =
        JSON.parse(localStorage.getItem("listadoCitas")) || [];
    const {
        register,
        handleSubmit,
        formState: { errors },
    ...
    const borrarCita = (nombreDuenio) => {
        const filtrarCita = citas.filter(
            (cita) => cita.duenio !== nombreDuenio
        );
                        ...
                        <Form.Group className="mb-3" controlId="sintomas">
                            ...
                            >
                                <Form.Control
                                    ...
                                    {...register("sintomas", {
                                        required:
                                            "La descripción de los sintomas es obligatorio",
                                    })}
                                />
                            </FloatingLabel>
                        </Form.Group>
                            ...
                            <Col className="text-center">
                                {errors.mascota && (
                                    <Alert variant="danger">
                                        {errors.mascota?.message}Minimo 2
                                        caracteres y como maximo 16
                                    </Alert>
                                )}
                                {errors.duenio && (
                                    <Alert variant="danger">
                                        {errors.duenio?.message}Minimo 2
                                        caracteres y como maximo 20
                                    </Alert>
                                )}
                                {errors.fecha && (
                                    <Alert variant="danger">
                                        {errors.fecha?.message}No puedes
                                        reservar una cita con la fecha anterior
                                    </Alert>
                                )}
                                {errors.hora && (
                                    <Alert variant="danger">
                                        {errors.hora?.message}Las citas son
